...
try {
  ...
  // El nombre vive en el `<button>` de la celda de persona y no en el texto de la fila: el texto
  // empieza por la inicial del avatar, y buscar "A" no filtra nada.
  const nombreDeFila = async (indice) =>
    (await filas.nth(indice).getByRole('button').first().innerText()).trim()
  ...
  await pagina.waitForFunction(
    (nombre) => (document.querySelector('tbody tr button')?.textContent ?? '').trim() !== nombre,
