...
try {
  ...
  await contexto.route('**/api/bff/**', async (ruta) => {
    ...
    if (peticion.method() === 'POST' && url.pathname.endsWith('/tasks/bulk')) {
      ...
      return ruta.fulfill({ json: { data: { aplicados: solicitudes.at(-1).ids.length, copiados: solicitudes.at(-1).ids.length } } })
    }
  ...
  assert.ok(await dialogo.getByRole('searchbox', { name: 'Buscar proyectos por nombre' }).isDisabled())
  ...
  await abrir()
  await pagina.waitForLoadState('networkidle')
  const buscador = dialogo.getByRole('searchbox', { name: 'Buscar proyectos por nombre' })
  await buscador.fill('no-existe')
  await dialogo.getByText('No hay proyectos con ese nombre.', { exact: true }).waitFor()
  await buscador.fill('pagina 2')
  await dialogo.getByRole('checkbox', { name: 'Proyecto página 2', exact: true }).check()
  assert.equal(await dialogo.getByRole('checkbox').count(), 1, 'Busca sin exigir tildes.')
  await buscador.fill('página 1')
  await dialogo.getByRole('checkbox', { name: 'Proyecto página 1', exact: true }).check()
  await dialogo.getByText('2 proyectos seleccionados:', { exact: true }).waitFor()
  await dialogo.getByRole('button', { name: 'Quitar Proyecto página 1', exact: true }).click()
  assert.ok(await dialogo.getByRole('checkbox', { name: 'Proyecto página 1', exact: true }).isChecked() === false)
  await dialogo.getByRole('checkbox', { name: 'Proyecto página 1', exact: true }).check()
  await buscador.fill('')
  assert.equal(await dialogo.getByRole('checkbox', { checked: true }).count(), 2)
  await dialogo.getByText(/se crearán 2 copias/).waitFor()
  ...
  await dialogo.getByRole('alert').waitFor()
  assert.equal(await dialogo.getByRole('checkbox', { checked: true }).count(), 2)
  assert.deepEqual(solicitudes[0], { ids, accion: 'project', valor: [2, 1] })
  ...
  console.info('Proyecto masivo: dos filas, búsqueda, selección múltiple, paginación, cancelación, carga/error/vacío, reintento, bloqueo, refresco y móvil correctos.')
} finally {
