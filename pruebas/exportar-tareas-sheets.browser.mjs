...
try {
  for (const movil of [false, true]) {
    ...
    assert.match(await dialogo.textContent(), /EXPORT_Ana Ríos/)
    assert.match(await dialogo.textContent(), /añadir libremente columnas y notas/)
    assert.doesNotMatch(await dialogo.textContent(), /bloqueos|fecha comprometida/)
