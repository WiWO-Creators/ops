...
export function MapaDeAreas (
  { areas, mias, sinArea, resumen, personas, personasPorId, onEntrar }: PropsMapa
) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {vacias.length > 0 && (
        <AreasVacias areas={vacias} mias={mias} onEntrar={onEntrar} />
      )}
...
/** Clases compartidas por las tarjetas, para que se comporten igual al pasar y al enfocar. */
...
function TarjetaDeArea (
  { area, propia, jefatura, gente, onEntrar }: {
          ...
          {areas.map((area) => (
            <li key={area.id}>
              <button
                ...
                className={cn(
                  'border-linea bg-superficie-elevada text-texto-tenue hover:text-texto hover:bg-hover',
                  'rounded-control ease-neo duration-rapida flex items-center gap-2 border px-2.5 py-1.5',
