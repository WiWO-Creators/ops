...
/**
 * Una persona dentro del organigrama: lo unico que la pantalla necesita de ella.
 *
 * No es `PersonaAsignable` ni `MiembroEquipo`: `GET /jerarquia` proyecta solo el id y el nombre,
 * porque el organigrama no muestra correos ni permisos y pedirlos seria exponer de mas.
 */
...
/**
 * Un area del organigrama del equipo, tal como viene dentro de `GET /jerarquia`.
 *
 * El arbol llega **plano**: cada fila dice de quien cuelga con `area_superior_id`, y anidarlas es
 * trabajo de quien las dibuja. No es lo mismo que `AreaDeCatalogo`, que son las features del
 * formulario de permisos, ni que `task_areas` de `Lookups`, que son las de la compañia.
 */
export interface AreaDelEquipo {
  id: number
  name: string
  /** `null` es una raiz del arbol: no cuelga de ninguna otra area. */
  area_superior_id: number | null
  /** Quien dirige el area. `null` si todavia no lo decidio nadie; el nombre sale de `asignables`. */
  jefe_staffid: number | null
  /**
   * Si quien pide puede escribir sobre esta area.
   *
   * Lo resuelve el backend por area y no la pantalla: quien no administra ve su rama entera pero
...
/**
 * Respuesta de `GET /jerarquia`: todo lo que la pantalla del organigrama necesita, de una vez.
 *
 * Es una sola peticion y no cuatro porque las cuatro listas cambian juntas — mover a alguien lo saca
 * de un area y lo mete en otra, o lo devuelve a `sin_area` —, y pedirlas por separado deja la
 * pantalla mostrando dos momentos distintos del mismo dato.
 *
 * La API responde **403** a quien no dirige nada y no administra, con un mensaje ya redactado.
 */
export interface Jerarquia {
  /** `false` mientras no haya ninguna area cargada: la pantalla se pinta vacia, no es un error. */
  hay_organigrama: boolean
  /** Distingue "veo todo el organigrama" de "veo solo mi rama". */
  es_admin: boolean
  areas: AreaDelEquipo[]
  /**
   * Quien no cuelga de ningun area.
   *
   * Hoy son las 184 personas de la instalacion, y vaciar esta lista es el trabajo que la pantalla
   * existe para permitir. Por eso tiene lugar propio en la interfaz y no un rincon.
   */
  sin_area: PersonaDeJerarquia[]
  /** Catalogo de personas activas para los selectores de jefe y para sumar gente. */
