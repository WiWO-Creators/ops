...
| **Checklist** | `Tasks_model::add_checklist_item()` (`:857`) y `update_checklist_order()` (`:887`); 418 ítems | `ListaChecklist.tsx` en el detalle: ver, agregar, tildar y quitar. Falta reordenar (`PUT`), renombrar un ítem y asignarlo | **Lista**: `POST /tasks/{id}/checklist`, `PUT` para reordenar y `PATCH\|DELETE` sobre un ítem (`V1.php:1582`) |
